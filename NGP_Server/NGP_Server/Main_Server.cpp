...
{
    // HP potion 임계 영역 init
    InitializeCriticalSection(&g_csHpPotion);

    // 현재 시간을 이용하여 난수 발생기를 초기화
    ...
    // 이벤트 생성
    // 클라 4개 접속 중일 때 0이 3번, 1이 0번, 2가 1번, 3이 2번, 이벤트 기다림
    for (int i = 0; i < 4; ++i)
    {
...
DWORD WINAPI ProcessClient(LPVOID arg)
{
    ...
    while (1)
    {
        ...
        // 게임 종료 시, 클라 1명만 남으면 기다리지 않고 진행
        // 밑의 if 조건들은 게임 종료 시 실행되는 조건!!
        if (g_iClientCount >= 2) {
            std::cout << iCurIndex << "가 " << g_iWaitClientIndex[iCurIndex] << "를 기다린다.." << std::endl;
        ...
        // 게임 진행 중에 다음 클라이언트의 스레드를 풀어주기 위함
        SetEvent(g_hClientEvent[iCurIndex]);
    }
...
bool SendRecv_PlayerInfo(SOCKET client_sock, int iIndex)
{
    ...
    { 
        ...
        g_tStoreData.team[iCurIndex] = TEAMNUM::TEAM2; 
    }
...
void CheckEnding(int iCurIndex)
{
    ...
    {
        // eNowTeam: 죽은 플레이어가 있는 팀이 어디 팀이냐
...
bool SendPlayerInit(SOCKET sock, int PlayerIndex)
{
    ...
    std::cout << "send player init... " << PlayerIndex << std::endl;
    return TRUE;
}
